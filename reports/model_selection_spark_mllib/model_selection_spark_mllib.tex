...
%Model selection for machine learning algorithms is a useful tool for a programmer who does not want to research machine learning, but still wants to use these algorithms in their work. This model selection algorithm for Apache Spark's MLlib library allows a user to practically use machine learning without knowing the intracacies of the theory.
\end{abstract}
...
% While this helps programmers who do not have much machine learning background use machine learning, there still is a problem: 
% with all the algorithms to choose from, each with their own separate parameters and assumptions, even with this abstraction it still takes up time to research which algorithm works the best.
%How about : due to the vast number of ML algos, one would still need to know which algo to use
...
%Thats a llllllllllllllllllloooooooooottttttttttttttttttttt of code
  %yes but it is the only code we have in the paper
  	%how bout split it up then or something. Its like a page and a half of straight code
...
%the way to tell how good a predictor is is by maximizing the explained variance and minimizing the root mean squared error (RMSE). 
